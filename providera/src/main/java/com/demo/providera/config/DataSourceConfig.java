package com.demo.providera.config;
 ...
 *
 * 启动时自动执行 schema.sql 建表并灌入初始数据，实现"开箱即有基础数据"。
 */
@Configuration
...
public class DataSourceConfig {
    ...
    private int port;
    @Value("${app.middleware.mysql.database:provider_a_db}")
    ...
    public DataSource dataSource() {
        ...
        ds.setConnectionTimeout(3000);  // 连不上快速失败，避免启动卡顿
        ds.setPoolName("provider-a-mysql");
        return ds;
    }
    ...
    public JdbcTemplate jdbcTemplate(DataSource dataSource) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            // 建表失败不阻断启动，运行时查询会回退到内存数据
            System.err.println("[provider-a] schema.sql 执行失败(后续查询将回退内存数据): " + e.getMessage());
        }
